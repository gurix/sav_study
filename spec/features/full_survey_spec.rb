...
feature 'Survey' do
  ...
  scenario 'A panel user fills in the whole survey', js: true do
    ...
    allow_any_instance_of(Subject).to receive(:created_at).and_return(20.minutes.ago)
  ...
  scenario 'A panel user fills in the whole survey to fast', js: true do
    allow_any_instance_of(Subject).to receive(:assigned_model).and_return("sav")
    ...
    expect(current_url).to eq 'https://s.cint.com/Survey/Finished?ProjectToken=d5ca5586-67d9-47a3-a2c2-e4a07f20db0d'
  end
end
